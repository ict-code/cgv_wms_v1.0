...
const schema = z.object({
  ...
  items: z.array(z.object({ itemId: z.string().min(1, "Required"), quantity: z.coerce.number().positive("Must be positive") })).min(1, "Add at least one item"),
});
type FormInput = z.input<typeof schema>;
type FormValues = z.infer<typeof schema>;

export function TransferCreateForm({ onClose }: { onClose: () => void }) {
  ...
  const createMutation = useMutation({
    mutationFn: async (values: FormValues) => (await apiClient.post("/transfers", values)).data,
    onSuccess: (data) => {
